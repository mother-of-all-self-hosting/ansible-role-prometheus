...
#
# Tags look like `v<Prometheus version>-<release>`:
#
# - if defaults/main.yml points at a Prometheus version that has never been
#   released, the release counter restarts at 0 (`v3.14.0-0`)
# - otherwise the counter is incremented (`v3.14.0-1`), but only if something
...
# `prometheus_version` is the leaf literal that Renovate bumps. Reading any of
# the variables derived from it (`prometheus_container_image_tag`,
# `prometheus_container_image`) would yield a Jinja expression rather than a
# version, and produce nonsense tags.
version="$(sed -nE 's|^prometheus_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Prometheus version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v3.14.0`),
